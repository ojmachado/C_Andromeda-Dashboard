...
export const DataTable: React.FC<{ data: InsightData[]; isLoading?: boolean }> = ({ data, isLoading }) => {
  ...
  const headers: { key: SortKey; label: string }[] = [
      ...
      { key: 'cpc', label: 'CPC' },
      { key: 'cpa', label: 'CPA' },
      // ROAS Removed from end or kept if needed, adjusting based on image request which ends at CPA
      // But prompt image doesn't show ROAS, let's check. Ah, image ends at CPA.
      // But ROAS is valuable. I will keep ROAS but move it before CPA or at end.
      // Image: NOME, STATUS, LINK, INVEST, MSGS, CUSTO/MSG, IMPR, CLICKS, CTR, CPC, CPA.
  ];
          ...
          {isLoading ? (
            ...
            sortedData.map((row, i) => (
              <tr key={i} className="border-b border-slate-100 dark:border-white/5 last:border-0 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                <td className="px-6 py-4 text-sm font-semibold truncate max-w-[280px]" title={row.name}>
                    ...
                    {row.detailsLink ? (
                        <a 
                            ...
                            className="text-slate-700 dark:text-white hover:text-primary hover:underline decoration-dashed underline-offset-4 transition-all block mb-1"
                        >
                            {row.name}
                        </a>
                    ) : (
                    ...
                    {!row.campaignName && row.objective && (
                        <div className="text-[9px] font-mono text-text-secondary mt-0.5 opacity-70">
